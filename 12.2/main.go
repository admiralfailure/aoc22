...
func Run() {
	...
	paths := dijkstra(nodeMap, target)

	shortestPath := 99999
	for i := range nodeMap {
		if nodeMap[i].name == 'S' || nodeMap[i].name == 'a' {
			dist := paths[i]
...
func getValidRoutes(routeMap []rune, idx int, mapWidth int) []int {
	...
	if idxLeft >= 0 && isRouteValid(routeMap[idxLeft], routeMap[idx]) {
		validRoutes = append(validRoutes, idxLeft)
	}
	if idxRight >= 0 && isRouteValid(routeMap[idxRight], routeMap[idx]) {
		validRoutes = append(validRoutes, idxRight)
	}
	if idxUp >= 0 && isRouteValid(routeMap[idxUp], routeMap[idx]) {
		validRoutes = append(validRoutes, idxUp)
	}
	if idxDown >= 0 && isRouteValid(routeMap[idxDown], routeMap[idx]) {
...
func dijkstra(graph []node, source int) []int {
	fmt.Println("Dijkstra:", source, string(graph[source].name))
